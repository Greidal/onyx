...
class NextcloudWebDAVClient:
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            logger.error(
                "Consider setting verify_ssl=False for self-signed certificates"
            )
            return False
  ...
  <d:prop>
    ...
    <d:displayname />
        ...
        try:
            ...
            logger.debug("WebDAV PROPFIND request successful")
    ...
    def _extract_file_info(
        ...
        try:
            ...
            # Extract properties
            propstat = response_elem.find(
                './/d:propstat[d:status="HTTP/1.1 200 OK"]', namespaces
            )
            if propstat is None:
                return None
            ...
            logger.warning(
                f"Failed to extract file info from response element: {e}"
            )
            return None
        ...
        if lastmod_elem is not None and lastmod_elem.text:
            try:
                ...
                dt = datetime.strptime(
                    lastmod_elem.text, "%a, %d %b %Y %H:%M:%S %Z"
                )
                properties["last_modified"] = dt.replace(tzinfo=timezone.utc)
            except ValueError:
                logger.warning(
                    f"Could not parse last modified date: {lastmod_elem.text}"
                )
    ...
    def _is_directory(
        self, prop_elem: Element, namespaces: dict[str, str]
    ) -> bool:
